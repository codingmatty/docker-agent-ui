...
export function ChatView({
  ...
  const displayMessages =
    streamingContent || isStreaming
      ? [...withContent, { role: 'assistant' as const, content: streamingContent }]
      : withContent;
          ...
          {displayMessages.map((msg, i) => {
            ...
            return (
              ...
              >
                ...
                {/* Content */}
                <div className={`flex-1 min-w-0 max-w-[85%] flex flex-col ${isUser ? 'items-end' : 'items-start'}`}>
                  <div className={`text-[9px] font-mono uppercase tracking-[0.22em] mb-1.5 ${
                  ...
                  {isUser ? (
                    ...
                    <div className={`border-l-2 pl-3 py-0.5 text-sm text-base-content/80 chat-message w-full transition-colors ${
                      isLastStreaming ? 'border-primary/50' : 'border-base-300'
                    }`}>
                      <MessageBody
                        content={msg.content ?? ''}
                        thoughts={msg.thoughts}
      ...
      <div className="px-4 py-3 border-t border-base-300 bg-base-200/40 shrink-0">
        <form onSubmit={handleSubmit} className="max-w-3xl mx-auto">
          <div className="agent-input-wrap flex items-center border border-base-300 rounded-sm bg-base-300/30">
            <span className="pl-3 text-primary/40 font-mono text-sm select-none shrink-0 leading-none">›</span>
            <input
              ref={inputRef}
              type="text"
